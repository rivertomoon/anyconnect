...
pj_status_t parse_turn_section(struct json_object *file_obj, CONFIG_t * config)
{
	...
	if( json_object_object_get_ex(lvl1_obj, "username", &lvl2_obj) && json_object_is_type(lvl2_obj, json_type_string) ) {  		config->turn_config.username = (char *)json_object_get_string(lvl2_obj);
	} else {
		goto on_error;
	}
...
pj_status_t parse_device_section(struct json_object *file_obj, CONFIG_t * config)
{
	struct json_object *lvl1_obj = NULL, *lvl2_obj = NULL;

	if( !json_object_object_get_ex(file_obj, "device", &lvl1_obj) )
		goto on_error;
		
	if( json_object_object_get_ex(lvl1_obj, "registered", &lvl2_obj) ) {
		if( strcmp(json_object_get_string(lvl2_obj), "true") == 0 )
...
pj_status_t parse_file(const char *filename, CONFIG_t * config)
{
	...
	if( parse_device_section(file_obj, config) != PJ_SUCCESS )
		goto on_error;
...
pj_status_t parse_message(const char *msg_buffer, CONFIG_t * config)
{
	...
	if( parse_device_section(msg_obj, config) != PJ_SUCCESS )
		goto on_error;
