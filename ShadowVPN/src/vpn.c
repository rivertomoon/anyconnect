...
void attach_ice_ctx(vpn_ctx_t *ctx, send_func_ptr_t func) {
	ctx->ice_tunnel_send = func;
}
              ...
              if (-1 == nat_fix_upstream(ctx->nat_ctx,
                ...
                continue;
					...
					if (-1 == tun_write(ctx->tun,
															ctx->tun_buf + SHADOWVPN_ZERO_BYTES + usertoken_len, 4)) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
              // do nothing
    ...
    if (FD_ISSET(ctx->tun, &readset)) {
      ...
      if (ctx->remote_addrlen) {
        ...
        r = sendto(sock_to_send, ctx->udp_buf + SHADOWVPN_PACKET_OFFSET,
				...
				/* r = iceagent_send_data(1, ctx->tun_buf + SHADOWVPN_PACKET_OFFSET,
    ...
    if (FD_ISSET(ctx->tun, &readset)) {
      ...
      if (ctx->remote_addrlen) {
        ...
        /* r = sendto(sock_to_send, ctx->udp_buf + SHADOWVPN_PACKET_OFFSET,
				...
				if( ctx->ice_tunnel_send != NULL )
					r = ctx->ice_tunnel_send(1, ctx->tun_buf, r);

        /* r = iceagent_send_data(1, ctx->tun_buf, r); */
...
size_t vpn_write_wrapper(vpn_ctx_t *ctx, void *buf, size_t size)
{
	if (-1 == tun_write(ctx->tun, (char *)buf, (unsigned)size) ) {
		if (errno == EAGAIN || errno == EWOULDBLOCK) {
			// do nothing
		} else if (errno == EPERM || errno == EINTR || errno == EINVAL) {
			// just log, do nothing
